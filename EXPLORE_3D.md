...
- **Boundary is code-based, not collider walls** (walls tunnel at run speed, esp. now that Shift sprints to 16). `BoundaryGuard` keeps you inside the union of zones (island circles + bridge rects), clamping to the nearest edge **removing only the outward velocity** (so you slide along edges). It also pushes you out of code-based **keep-outs**: archway posts (`POST_OBSTACLES`, circles) and tunnel walls (`WALL_OBSTACLES`, line barriers within the wall's run) — needed because those thin shapes sit inside the wide island circles where the zone check won't stop you. Falls below y=-8 respawn at `SPAWN`.
