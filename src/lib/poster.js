/* ------------------------------------------------------------------
 * Generated "poster" art for items that have no thumbnail (internal tools,
 * leadership work, image-less projects). Pure 2D-canvas — NO three.js — so the
 * normal site can import it (the 3D world wraps the canvas in a texture; see
 * explore/screenPoster.js).
 *
 * Each poster is a 16:9 image: a dark gradient ground, a decorative motif, a
 * category eyebrow, and the title in bold. Palette + motif are chosen
 * deterministically from the item's key, so a given item looks identical on its
 * in-world TV and on the website, yet every item looks distinct.
 * ------------------------------------------------------------------ */
...
// Draw a poster onto a fresh canvas and return it.
...
// Poster as a PNG data URL, for plain <img> tags (cards, panels).
